...
async function searchNearest() {
  ...
  for (const provider of providers) {
    const geo = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(provider.address)}`, {
    headers: {
      'Accept': 'application/json',
      'User-Agent': 'LOGIKART/1.0 (contact@logikart.app)'
    }
...
function loadProvidersFromLocalStorage() {
  ...
  setTimeout(() => geocodeAndAddToMap(provider), index * 1500);
});
  updateProviderList();
}
...
function generatePDF() {
  ...
  // Injecte le contenu HTML dans le bloc caché
  reportContent.innerHTML = `
    <div style="font-family: Arial, sans-serif; padding: 20px; color: #000;">
      <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #004080; padding-bottom: 10px;">
        <img src="logikart-logo.png" alt="LOGIKART" style="height: 50px;">
  ...
  // ➤ Affiche temporairement le contenu pour html2pdf
  ...
  // ➤ Génére et télécharge le PDF, puis nettoie
  html2pdf().set(opt).from(reportContent).save().then(() => {
    reportContent.style.display = "none";              // Cache à nouveau
    form.reset();                                      // Réinitialise le formulaire
    closeReportForm();                                 // Ferme la fenêtre modale
  });
}
function openItineraryTool() {
...
async function calculateRoute() {
  ...
  const orsRes = await fetch('https://api.openrouteservice.org/v2/directions/driving-car/geojson', {
    method: 'POST',
    headers: {
      'Authorization': 'eyJvcmciOiI1YjNjZTM1OTc4NTExMTAwMDFjZjYyNDgiLCJpZCI6ImQ4YTg5NTg4NjE0OTQ5NjZhMDY3YzgxZjJjOGE3ODI3IiwiaCI6Im11cm11cjY0In0=', // 🔁 remplace ici avec ta vraie clé
...
function populateTechnicianSuggestions() {
  ...
  providers.forEach(p => {
    ...
    option.value = `${p.firstName} ${p.contactName}`;
